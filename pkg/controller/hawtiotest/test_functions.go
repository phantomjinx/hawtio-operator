...
func loadExpectedFile(filename string, platform string, obj runtime.Object) {
	platformFilename := fmt.Sprintf("%s-%s", strings.ToLower(platform), filename)
	platformPath := filepath.Join("testdata", fmt.Sprintf("%s.yml", platformFilename))
...
func performDeleteHawtioCR(name string, namespace string) {
...
func performEmptyTypeHawtioCR(ctx context.Context) {
	...
	DeferCleanup(func() {
		...
		performDeleteHawtioCR(emptyTypeName, HawtioNamespace)
	})

	Eventually(func(g Gomega) {
		fmt.Println("=== RUNNING EVENTUALLY ===")
		...
		fmt.Println("=== FINISHED EVENTUALLY ===")
	}, timeout, interval).Should(Succeed())
}

func performCommonResourceTest(ctx context.Context, platform string) {
